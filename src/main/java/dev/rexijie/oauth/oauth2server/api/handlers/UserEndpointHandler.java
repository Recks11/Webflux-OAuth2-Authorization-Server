...
@Component
public class UserEndpointHandler extends ApiEndpointHandler {
    ...
    public Mono<ServerResponse> saveUser(ServerRequest request) {
        return bodyToMono(request, UserDTO.class)
                .flatMap(userService::addUser)
                .flatMap(userDTO -> ServerResponse.status(HttpStatus.CREATED).bodyValue(userDTO))
                .doOnError(err -> {
                    LOG.error("error handling request {} because: {}", request.path(), err.getMessage());
